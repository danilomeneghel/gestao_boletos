...
$banco = mysqli_query("SELECT * FROM bancos WHERE id_banco='3'")or die (mysqli_error());
...
$compra = mysqli_query("SELECT *,date_format(data_venci, '%d/%m/%Y') AS datad, date_format(data, '%d/%m/%Y') AS data FROM faturas WHERE id_venda='$id_venda' AND situacao !='B'")or die (mysqli_error());
...
$cli = mysqli_query("SELECT * FROM cliente WHERE id_cliente='$idcliente'")or die (mysqli_error());
...
$up = mysqli_query("UPDATE faturas SET nosso_numero ='$numero', banco ='$banco' WHERE id_venda='$id_venda'") or die(mysqli_error());
?>
