...
$banco = mysqli_query($conexao,"SELECT * FROM bancos WHERE id_banco='4'")or die (mysqli_error());
...
$compra = mysqli_query($conexao,"SELECT *,date_format(data_venci, '%d/%m/%Y') AS datad, date_format(data, '%d/%m/%Y') AS data FROM faturas WHERE id_venda='$id_venda' AND situacao !='B'")or die (mysqli_error());
...
$res = $d[0];
$banco = "ITAU";
$up = mysqli_query($conexao,"UPDATE faturas SET nosso_numero ='$res', banco ='$banco' WHERE id_venda='$id_venda'") or die(mysqli_error());
?>
