...
// ************** SELECIONAR DADOS **************************
function seleciona($conexao,$sql){
...
function inserir($conexao,$tabela, $dados){
	...
	if($numCampo == $numValores){ // if insert
		...
		$SQL = substr_replace($SQL, ")", -2, 1);
		$insere = mysqli_query($conexao,$SQL)or die(mysqli_error($conexao));
		return $insere;
	}else{
...
// ******************* ALTERAR DADOS ***************************
function alterar($conexao,$tabela, $dados, $string){
	// PEGAR CAMPOS DA ARRAY
	$arrCampo = array_keys($dados);
	//PEGAR VALORES DA ARRAY
	$arrValores = array_values($dados);
