...
<body>
	<div id="entrada">
		...
		<?php
			...
			$id = $_GET['id_venda'];
			$sql= mysqli_query($conexao,"SELECT *,date_format(data_venci, '%d/%m/%Y') AS data_v,date_format(dbaixa, '%d/%m/%Y') AS dtabaixa FROM faturas WHERE id_venda='$id'");
			$ver = mysqli_fetch_array($sql);
			$juros = $ver['valor_recebido'] - $ver['valor'];
		?>

		<div id="forms">
			<div id="dados">ID: <strong><?php echo $ver['id_venda']; ?></strong></div>
