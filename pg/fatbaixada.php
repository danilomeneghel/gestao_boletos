	...
	<div id="forms">
      ...
      if($_SESSION['id_usuario_session'] == 1) {
  			if(isset($_POST['pesquizar']) && $_POST['pesquizar'] != ""){
			       ...
			       $sql_1 = "SELECT * ,date_format(dbaixa, '%d/%m/%Y') AS data FROM faturas WHERE situacao ='B' AND (nome LIKE '%$pesquisar%' OR id_venda LIKE '%$pesquisar%' OR nosso_numero LIKE '%$pesquisar%')";
  			}
        ...
        if(isset($_POST['pesquizar']) && $_POST['pesquizar'] != ""){
          $pesquisar = $_POST['pesquizar'];
          $sql_1 = "SELECT * ,date_format(dbaixa, '%d/%m/%Y') AS data FROM faturas WHERE id_cliente='$id_cliente' AND situacao ='B' AND (nome LIKE '%$pesquisar%' OR id_venda LIKE '%$pesquisar%' OR nosso_numero LIKE '%$pesquisar%')";
        }
        elseif(isset($_POST['datai']) && $_POST['datai'] and $_POST['dataf'] != ""){
          $datai = implode("-",array_reverse(explode("/",$_POST['datai'])));
          $dataf = implode("-",array_reverse(explode("/",$_POST['dataf'])));
          $sql_1 = "SELECT * ,date_format(dbaixa, '%d/%m/%Y') AS data FROM faturas WHERE id_cliente='$id_cliente' AND situacao ='B' AND dbaixa BETWEEN ('$datai') AND ('$dataf')";
        }else{
          $sql_1 = "SELECT * ,date_format(dbaixa, '%d/%m/%Y') AS data FROM faturas WHERE id_cliente='$id_cliente' AND situacao ='B' ORDER BY dbaixa ASC";
        }
      }
						...
						<table width="100%" border="0" cellspacing="1" cellpadding="5" data-rowindex="1" data-rowtype="1">
						  <tr>
						    <th width="3%"><input type="checkbox" name="todos" id="todos" value="todos" onclick="marcardesmarcar();" /></th>
								<th width="7%"><span class="fontebranca">ID</span></th>
						    <th width="23%"><span class="fontebranca">Nome</span></th>
						    <th width="27%"><span class="fontebranca">Descrição</span></th>
						    <th width="9%" align="center"><span class="fontebranca">Data Baixada</span></th>
						    <th width="8%" align="center"><span class="fontebranca">Valor</span></th>
						    <th width="7%" align="center"><span class="fontebranca">Residual</span></th>
