...
<div id="entrada">
	...
	<div id="forms">
      ...
      if($_SESSION['id_usuario_session'] == 1) {
  			...
  			$sql_1 = "SELECT * ,date_format(data_venci, '%d/%m/%Y') AS data FROM faturas WHERE situacao ='P' AND nome LIKE '%$pesquisar%' OR nosso_numero LIKE '%$pesquisar%'";
  			}
  			...
  			if(isset($_POST['pesquizar']) && $_POST['pesquizar'] != ""){
  			$pesquisar = $_POST['pesquizar'];
  			$sql_1 = "SELECT * ,date_format(data_venci, '%d/%m/%Y') AS data FROM faturas WHERE id_cliente='$id_cliente' AND situacao ='P' AND nome LIKE '%$pesquisar%' OR nosso_numero LIKE '%$pesquisar%'";
  			}
  			elseif(isset($_POST['datai']) && $_POST['datai'] and $_POST['dataf'] != ""){
  			$datai = implode("-",array_reverse(explode("/",$_POST['datai'])));
  			$dataf = implode("-",array_reverse(explode("/",$_POST['dataf'])));
  			$sql_1 = "SELECT * ,date_format(data_venci, '%d/%m/%Y') AS data FROM faturas WHERE id_cliente='$id_cliente' AND situacao ='P' AND data_venci BETWEEN ('$datai') AND ('$dataf')";
  			}else{
  			$sql_1 = "SELECT * ,date_format(data_venci, '%d/%m/%Y') AS data FROM faturas WHERE id_cliente='$id_cliente' AND situacao ='P' ORDER BY data_venci ASC";
  			}
      }
			...
			<div id="tabela1" style="width:98%;">
              ...
              <?php } ?>
						...
						// Executa o Query
						$sql_query = mysqli_query($conexao,$sql_select) or die(mysqli_error());
              ...
              <td align="center" valign="middle">
							...
							<td width="36" align="center">
								<div id="editar">
									<a href="pg/editafatura.php?id_venda=<?php echo $array['id_venda'] ?>&pg=fatpendente" class="editar" title="Editar dados"
              ...
              <?php } ?>
                ...
                <?php } ?>
								...
								<?php
                if($_SESSION['id_usuario_session'] == 1) {
				          $sqlsoma = mysqli_query($conexao,"SELECT sum(valor) AS val FROM faturas WHERE situacao ='P'");
                } else {
    							$sqlsoma = mysqli_query($conexao,"SELECT sum(valor) AS val FROM faturas WHERE id_cliente='$id_cliente' AND situacao ='P'");
                }
