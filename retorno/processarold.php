...
/* $sql = mysqli_query($conexao,"INSERT INTO financeiro (nosso_numero) VALUES('$nm')ON DUPLICATE KEY UPDATE nosso_numero= '$nm'")or die(mysqli_error()) ;
$id_res = mysql_insert_id();
//echo $id_res.'<br/>';	  
$contar = mysqli_query($conexao,"SELECT * FROM financeiro WHERE id='$id_res'") or die(mysqli_error());
$dado = mysqli_fetch_array($contar);
	$bosta = $dado['nosso_numero'];
	echo $bosta; */
  	$sql = mysqli_query($conexao,"UPDATE faturas SET lote='$lote' ,dbaixa = '$dbaixa', situacao = 'B' WHERE nosso_numero = '$nm'") or die(mysqli_error());

	}
echo "<br/>";
}
/*  $sql = mysqli_query($conexao,"SELECT dbaixa FROM faturas ORDER BY dbaixa DESC LIMIT 1");
$v = mysqli_fetch_array($sql);
$valordbaixa = $v['dbaixa'];

$contar = mysqli_query($conexao,"SELECT dbaixa FROM faturas WHERE dbaixa = '$valordbaixa'");
$dado = mysqli_num_rows($contar);
			print "
