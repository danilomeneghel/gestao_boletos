...
<table width="100%" border="0" cellspacing="0" cellpadding="3" class="bordasimples">
  ...
  <?php 
  $rel = mysqli_query($conexao,"SELECT * FROM financeiro ORDER BY nosso_numero ASC") or die(mysqli_error());
  while($result = mysqli_fetch_array($rel)){
	  $nosso = $result['nosso_numero'];
	  $pg = mysqli_query($conexao,"SELECT *,date_format(data_venci, '%d/%m/%Y') AS datav FROM faturas WHERE nosso_numero ='$nosso'");
	  $nomes = mysqli_fetch_array($pg); 
	  if($nomes['nome'] != ""){
	  ?>
...
<?php 
$limpa = mysqli_query($conexao,"TRUNCATE TABLE financeiro");
